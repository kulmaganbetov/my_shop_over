...
TOOLS = [
    {
        ...
        "description": "Поиск товаров в каталоге по запросу, категории или фильтрам цены. "
                       ...
                       "ВАЖНО: Если пользователь спрашивает о конкретном товаре (модель, артикул, название), "
                       "вопросы 'в наличии?', 'цена?', 'есть?' - ОБЯЗАТЕЛЬНО используй этот инструмент!",
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Поисковый запрос (название товара, модель, категория, бренд). Передавай полное название модели!"
                },
                "category": {
                    "type": "string",
    ...
    },
    {
        ...
        "description": "Получить альтернативные варианты для компонента сборки. "
                       ...
                       "ВАЖНО: Если пользователь НЕДОВОЛЕН показанными альтернативами и просит другие "
                       "(например 'не нравится AFOX', 'хочу MSI', 'нужен производитель с именем') - "
                       "ОБЯЗАТЕЛЬНО вызови этот инструмент СНОВА с новым preference!",
        "parameters": {
            "type": "object",
            "properties": {
                "component_type": {
                ...
                "preference": {
                    "type": "string",
                    "description": "Предпочтение пользователя. Примеры: 'дешевле', 'дороже', 'Intel', 'AMD', "
                                   "'MSI', 'ASUS', 'Gigabyte', 'не устраивает AFOX', 'хочу производителя с именем'. "
                                   "ПЕРЕДАВАЙ ПОЛНЫЙ ТЕКСТ ЗАПРОСА ПОЛЬЗОВАТЕЛЯ!"
                }
            },
